export default class BrainlockApp {
  ...
  qs(){ const g=id=>document.getElementById(id); return {
    ...
    importBtn:g('importBtn'), importCfg:g('importCfg'), resetCfg:g('resetCfg'),
    soundBtn:g('soundBtn')
  };}
  bind(){ const U=this.ui;
    ...
    U.dbgBtn.onclick=()=>{ U.dbgDlg.showModal(); this.dumpWhy(); this.dumpOptions(); };
    U.closeDbg.onclick=()=>U.dbgDlg.close();
    // Config import/reset may exist in config; if not, ignore
    if (U.importBtn && U.importCfg && U.resetCfg) {
      U.importBtn.onclick=()=>U.importCfg.click();
      U.importCfg.onchange=async(e)=>{ const file=e.target.files?.[0]; if(!file) return; const txt=await file.text(); try{ const obj=JSON.parse(txt); localStorage.setItem('brainlock:configOverride', JSON.stringify(obj)); alert('Config imported. Restart session to apply.'); }catch(err){ alert('Invalid JSON'); } };
      U.resetCfg.onclick=()=>{ localStorage.removeItem('brainlock:configOverride'); alert('Config reset to built-in. Restart session to apply.'); };
    }
    // Sound toggle
    U.soundBtn.onclick=()=>{ this.enableSound(true); alert('Sounds ON. I will DING loudly when the session ends.'); };
    // Try to restore sound state
    if(localStorage.getItem('brainlock:sound')==='1'){ this.enableSound(false); }
  }
  // --------- Sound ---------
  enableSound(userGesture){
    if(this._audioReady) return;
    const AC = window.AudioContext || window.webkitAudioContext;
    if(!AC) return;
    this.ac = new AC();
    this.master = this.ac.createGain();
    this.master.gain.value = 1.0; // LOUD
    this.master.connect(this.ac.destination);
    const resume = ()=>{ if(this.ac.state==='suspended') this.ac.resume(); };
    if(userGesture) resume();
    document.body.addEventListener('touchstart', resume, {once:true});
    document.body.addEventListener('click', resume, {once:true});
    this._audioReady = true;
    localStorage.setItem('brainlock:sound','1');
  }
  tone(freq=880, dur=0.2, vol=1, type='sine'){
    if(!this._audioReady) return;
    ...
    g.gain.linearRampToValueAtTime(0.25*vol, now+0.01);
  ...
  ding(){
    // two-note chime
    this.tone(880, 0.18, 1, 'triangle');
    setTimeout(()=>this.tone(1320, 0.14, 1, 'sine'), 130);
  }
  buzz(){
    this.tone(120, 0.28, 1, 'square');
  }
  // -------------------------
